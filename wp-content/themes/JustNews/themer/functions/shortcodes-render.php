...
function wpcom_sc_tabs($atts){
    if(isset($atts['id'])){
        ...
        if(isset($data['title']) && is_array($data['title'])) {
            ...
            $i = 0;
            $minHeight = (isset($data['type'])&&$data['type']?' style="min-height:'.(43*count($data['title'])-2).'px;"':'');
            if(isset($data['body']) && is_array($data['body'])) {
                foreach ($data['body'] as $body) {
                    $html .= '<div role="tabpanel" class="tab-pane fade' . ($i == 0 ? ' in active' : '') . '" id="tabs-' . $atts['id'] . '-' . $i . '"' . $minHeight . '>' . do_shortcode($body) . '</div>';
                    $i++;
                }
            }
            $html .= '</div></div></div>';
...
function wpcom_sc_map($atts, $content=''){
    if(isset($atts['pos'])){
        ...
        $html .= baidu_map($str, $atts['pos'], $atts['scroll'], false);
        $html .= '</div>';
        return $html;
    }
}