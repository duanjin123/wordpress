                    ...
                    // Else, we resize the image and return the new resized image url.
                        ...
                        if ( ! is_wp_error( $resized_file ) ) {
                            ...
                            if(!$img_id){
                                ...
                                $img_id = $wpdb->get_var("SELECT post_id FROM `$postmeta_table` WHERE meta_key = '_wp_attached_file' AND meta_value = '$value'");
                            }
                            ...
                            if($img_id){
                                ...
                                if(is_array($img_data)){
                                    ...
                                    if( isset($img_data['sizes'][$size]) && isset($img_data['sizes'][$size]['file'])){ // 此处已经重新生成了缩略图，需要删除之前的缩略图片
                                        $delete = 1;
                                        foreach($img_data['sizes'] as $s => $img){ // 检查是否有同尺寸，避免误删
