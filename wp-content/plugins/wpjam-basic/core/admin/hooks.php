...
add_filter('wpjam_html_replace', function ($html){
	...
	return str_replace(
		...
		[
			'/wp-content/plugins/wpjam-basic/core/static/load-scripts.php?v='.$v.'&',
			'/wp-content/plugins/wpjam-basic/core/static/load-styles.php?v='.$v.'&'
		], 
		$html
	);
});

define('WPJAM_BASIC_CORE_STATIC_DIR', str_replace(ABSPATH, '/', WPJAM_BASIC_PLUGIN_DIR).'core/static/');

add_action('wp_default_scripts', function($scripts){
	$scripts->default_dirs = array_merge($scripts->default_dirs, [
		'/wp-includes/js/thickbox/',
		'/wp-includes/js/mediaelement/', 
		WPJAM_BASIC_CORE_STATIC_DIR
	]);
});

add_action('wp_default_styles', function($styles){
	$styles->default_dirs = array_merge($styles->default_dirs, [
		'/wp-includes/js/thickbox/', 
		'/wp-includes/js/mediaelement/', 
		WPJAM_BASIC_CORE_STATIC_DIR
	]);
...
add_action('admin_enqueue_scripts', function(){
	...
	wp_enqueue_script('raphael', WPJAM_BASIC_CORE_STATIC_DIR.'raphael.min.js');
	wp_enqueue_script('morris', WPJAM_BASIC_CORE_STATIC_DIR.'morris.min.js');

	// wp_enqueue_style('morris', '//cdn.staticfile.org/morris.js/0.5.1/morris.css');
	wp_enqueue_style('morris', WPJAM_BASIC_CORE_STATIC_DIR.'morris.css');
	wp_enqueue_style('wpjam-style', WPJAM_BASIC_CORE_STATIC_DIR.'style.css');
	...
	if($pagenow != 'plugins.php'){
		wp_enqueue_script('wpjam-script', WPJAM_BASIC_CORE_STATIC_DIR.'script.js', array('jquery'), '', true);
		wp_enqueue_script('wpjam-form', WPJAM_BASIC_CORE_STATIC_DIR.'form.js', array('jquery'), '', true);
	}
...
add_action('print_media_templates', function (){?>
	...
	<img style="{{ data.img_style }}" src="{{ data.img_url }}?imageView2{{ data.thumb_args }}" alt="" /><a href="javascript:;" data-bg_class="{{ data.bg_class }}"  class="del-img dashicons dashicons-no-alt"></a>
	</script>
