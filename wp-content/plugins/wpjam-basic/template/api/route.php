...
if($json == 'token'){
	include(WPJAM_BASIC_PLUGIN_DIR.'template/api/token.php');
	exit;
}

do_action('wpjam_api_template_redirect', $json);

$api_setting	= wpjam_get_api_setting($json);
if(!$api_setting){
	wpjam_send_json([
...
foreach ($api_setting['modules'] as $module){
	...
	if($type == 'post_type'){
		...
		if($action == 'list'){
			$template	= WPJAM_BASIC_PLUGIN_DIR.'template/api/post.list.php';
		}elseif($action == 'get'){
			$template	= WPJAM_BASIC_PLUGIN_DIR.'template/api/post.get.php';
		}
	}elseif($type == 'taxonomy_post_type'){
		$template	= WPJAM_BASIC_PLUGIN_DIR.'template/api/taxonomy.post.list.php';
	}elseif($type == 'taxonomy'){
		$template	= WPJAM_BASIC_PLUGIN_DIR.'template/api/taxonomy.list.php';
	}elseif($type == 'setting'){
		$template	= WPJAM_BASIC_PLUGIN_DIR.'template/api/setting.php';
	}elseif($type == 'other'){
		$template	= WPJAM_BASIC_PLUGIN_DIR.'template/api/other.php';
	}
