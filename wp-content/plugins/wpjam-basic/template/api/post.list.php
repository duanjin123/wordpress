...
if(!$is_sub){
	if($post_type){
		$response['post_type_title']	= $post_type_setting['label'];
	}
	
	if($cursor	= wpjam_get_parameter('cursor',	array('default'=>0,	'type'=>'int'))){
		$wp->set_query_var('date_query', array(
			array('before' => get_date_from_gmt(date('Y-m-d H:i:s',$cursor)))
		));
	}

	if($since	= wpjam_get_parameter('since',	array('default'=>0,	'type'=>'int'))){
		$wp->set_query_var('date_query', array(
			array('after' => get_date_from_gmt(date('Y-m-d H:i:s',$since)))
		));
	}
