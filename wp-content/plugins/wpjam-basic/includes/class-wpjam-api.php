...
abstract Class WPJAM_API{
	...
	public static function get_parameter($parameter, $args=array()){
		...
		if ($method == 'POST') {
			...
			$value = $_REQUEST[$parameter] ?? $default;
		}
	...
	public static function parse_fields_setting($fields, $is_sub=0){
		...
		if(is_array($fields)){
			...
			foreach ($fields as $field_key => $field_detail) {
				...
				$field_detail	= self::parse_shortcode_attr(stripslashes_deep($field_detail), 'field');
			...
			foreach (explode("\n", $fields) as $i => $a_field) {
				if(trim($a_field)){
					$a_field	= self::parse_shortcode_attr(stripslashes_deep($a_field), 'field');
	...
	public static function parse_field_value($value, $field){
		...
		if($field['type'] == 'img'){
			...
			if($item_type == 'url'){
				$value	= wpjam_get_thumbnail(wpjam_urlencode_img_cn_name($value), $thumbnail_size);
			}else{
			...
			if(is_array($value)){
				...
				$item_type		= $field['item_type']??'';
				if($item_type == 'url'){
					$value	= array_map(function($v) use ($thumbnail_size){ return  wpjam_get_thumbnail(wpjam_urlencode_img_cn_name($v), $thumbnail_size); }, $value);
				}else{
				...
				foreach ($field['fields'] as $sub_key => $sub_field) {
					if($sub_field['type']	== 'img'){
						...
						array_walk($value, function(&$v) use ($sub_key, $thumbnail_size, $item_type){ 
							if($item_type == 'url'){
								$v[$sub_key] = wpjam_get_thumbnail(wpjam_urlencode_img_cn_name($v[$sub_key]), $thumbnail_size);
							}else{
						...
						array_walk($value, function(&$v) use ($sub_key, $thumbnail_size, $item_type){ 
							if($item_type == 'url'){
								$v[$sub_key] = array_map(function($a_v) use ($thumbnail_size){ return  wpjam_get_thumbnail(wpjam_urlencode_img_cn_name($a_v), $thumbnail_size); }, $v[$sub_key]);
							}else{
						...
						array_walk($value, function(&$v) use ($sub_key){ $v[$sub_key] = wpjam_get_thumbnail(wpjam_urlencode_img_cn_name($v[$sub_key])); });
					}else{
			...
			if(!empty($field['data_type'])){
				if($field['data_type'] == 'video' || $field['data_type'] == 'qq-video' || $field['data_type'] == 'qq_video'){
					$value	= self::get_video_mp4($value);
	...
	public static function get_video_mp4($id_or_url){
		if(filter_var($id_or_url, FILTER_VALIDATE_URL)){ 
			if(preg_match('#http://www.miaopai.com/show/(.*?).htm#i',$id_or_url, $matches)){
				...
				return wpjam_urlencode_img_cn_name($id_or_url);
			}
		}else{
	...
	public static function get_appid(){
		$wpjam_grant	= get_option('wpjam_grant');
		$wpjam_grant	= $wpjam_grant?:[];

		if(empty($wpjam_grant) || empty($wpjam_grant['appid'])){
			$wpjam_grant['appid']	= 'jam'.strtolower(wp_generate_password(15, false, false));

			update_option('wpjam_grant', $wpjam_grant);
		}

		return $wpjam_grant['appid'];
	}

	public static function reset_secret(){
		$wpjam_grant	= get_option('wpjam_grant');
		$wpjam_grant	= $wpjam_grant?:[];

		$secret	= strtolower(wp_generate_password(32, false, false));

		$wpjam_grant['secret']	= md5($secret);
	...
	public static function generate_access_token($appid, $secret){
		...
		if(empty($wpjam_grant) || $wpjam_grant['appid'] != $appid){
			return new WP_Error('invalid_appid', '非法AppId');
		}

		if(empty($wpjam_grant['secret']) || $wpjam_grant['secret'] != md5($secret)){
		...
		$wpjam_grant['token']	= $token;
		$wpjam_grant['time']	= $time;
	...
	public static function validate_access_token($token){
		...
		if(empty($wpjam_grant) || empty($wpjam_grant['token']) || $wpjam_grant['token']!=$token || (time()-$wpjam_grant['time'] > 7200)){
			return false;
		}
	...
	public static function is_over_quota($json, $max_times=1000){
		$today			= date('Y-m-d', current_time( 'timestamp' ));
		$current_times	= wp_cache_get($json.'_'.$today, 'wpjam_api_times');
		$current_times	= $current_times?:0;

		if($current_times < $max_times){
			wp_cache_set($json.'_'.$today, $current_times+1, 'wpjam_api_times', DAY_IN_SECONDS);
			return false;
		}else{
			return true;
		}
	...
	public static function add_direct($json, $template){
		if(self::get_direct($json)) return true;
		
		$direct_api		= get_option('direct-api')?:array();
		$direct_api[]	= compact('json', 'template');

		update_option('direct-api', $direct_api);
	}

	public static function get_direct($json){
		$direct_api		= get_option('direct-api')?:array();

		if(!$direct_api) return false;

		foreach ($direct_api as $api) {
			if(isset($api['json']) && $api['json'] == $json){
				return WP_CONTENT_DIR.$api['template'];
			}
		}
