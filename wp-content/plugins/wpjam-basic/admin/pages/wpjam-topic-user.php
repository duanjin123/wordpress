...
function wpjam_topic_user_ajax_response(){
	...
	if($action == 'delete_weixin_user'){
		...
		$response		= wpjam_remote_request('http://jam.wpweixin.com/api/user.json', [
			'method'	=> 'POST',
			'body'		=> $data,
		...
		if(is_wp_error($response)){
			return wpjam_send_json($response);
		}

		$response['last_update']	= time();

		update_user_meta(get_current_user_id(), 'wpjam_weixin_user', $response);
