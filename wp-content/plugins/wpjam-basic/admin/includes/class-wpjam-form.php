...
class WPJAM_Form{
	...
	public static function get_field_html($field){
		...
		switch ($type) {
			...
			case 'image':
				$field_html	= self::get_input_field_html('url', $name, $key, $class, $value, $extra, $description).'<input type="button" item_type="image" class="wpjam-file button" value="选择图片">';
				break;
			...
			case 'img':
				...
				if(isset($field['size'])){
					...
					if($size['width'] && $size['height']){
						$width		= $size['width'];
						$height		= $size['height'];
						$thumb_args	= '/1/2/w/'.$width.'/h/'.$height;

						$width		= intval($width/2);
						$height		= intval($height/2);
						$img_style	= 'width:'.$width.'px;'.' height:'.$height.'px;';
					}elseif($size['width']){
						$width		= $size['width'];
						$thumb_args	= '/2/2/w/'.$width;
						$width		= intval($width/2);
						...
						$height		= intval($height/2);
						$thumb_args	= '/2/2/h/'.$height;
					}
				}

				$img_style	= $img_style?:'max-width:200px;';
				$thumb_args	= $thumb_args?:'/2/w/400/';
