...
class WPJAM_AdminTopic  {
	...
	public static function item_callback($item){
		...
		$item['title']		= $wpjam_list_table->get_row_action('reply',['id'=>$item['id'],'title'=>wp_unslash($item['title']).$reply_count]);
	...
	public static function get_fields($action_key='', $id=0){
		if($action_key == ''){
			...
			$topic_title	= '<h2>'.convert_smilies(wp_unslash($wpjam_topic['title'])).'</h2>';
