...
function wpjam_update_dashboard_widget_callback(){
	...
	<style type="text/css">
		...
		#dashboard_wpjam .rss-widget {font-size: 13px; padding: 12px; }
		#dashboard_wpjam .rss-widget a{font-weight: 400; }
	</style>
	...
	if($jam_posts){
		...
		foreach ($jam_posts as $jam_post){
			if($i == 5) break;
			echo '<li><a href="http://blog.wpjam.com'.$jam_post['post_url'].'">'.$jam_post['title'].'</a></li>';
			$i++;
		}
	}	
	?>
		...
		$wpjam_options['wpjam_custom_footer_box'] = [
			...
			'fields'	=> [
				'custom_footer'=>[
					'title'=>'', 
					'type'=>'textarea', 
					'description'=>'自定义日志 Footer 代码可以让你在当前日志插入独有的 JS，CSS，iFrame 等类型的代码，让你可以对具体一篇日志设置不同样式和功能，展示不同的内容。'
				]
			]
		];
	}
