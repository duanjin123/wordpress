...
class UM_Setup {
	...
	function __construct() {
		...
		$this->core_pages = array(
			'user' => array( 'title' => '用户信息' ),
			'login' => array( 'title' => '登录' ),
			'register' => array( 'title' => '注册' ),
			'members' => array( 'title' => '用户列表' ),
			'logout' => array( 'title' => '登出' ),
			'account' => array( 'title' => '我的资料' ),
			'password-reset' => array( 'title' => '重置密码')
		);

		$this->core_pages = apply_filters('um_core_pages_info', $this->core_pages );
			...
			foreach($this->core_pages as $slug => $array ) {
				...
				**/
				...
				if ( !$page_exists ) {
					...
					if( $slug == 'add-post' ){
						update_post_meta($post_id, 'page_template', 'page-editor.php');
					}
