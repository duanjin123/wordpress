...
class UM_User_posts {
	...
	***/
	function load_posts( $args ) {
		global $ultimatemember, $is_author;
		...
		$is_author = 0;
		if($current_user->ID && $author == $current_user->ID ){
			$is_author = 1;
			$ultimatemember->shortcodes->loop = $ultimatemember->query->make("post_type=$post_type&post_status=draft,pending,publish&posts_per_page=$posts_per_page&offset=$offset&author=$author");
		}else{
