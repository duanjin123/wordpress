<?php
	...
	function um_submit_account_details( $args ) {
		...
		if ( isset($_POST['user_password']) && $_POST['user_password'] && $_POST['confirm_user_password'] ) {
			...
			if ( isset($_POST['um_account_submit'])   ) {
				...
				if( $current_tab != 'password' ){
					...
					if ( isset($_POST['nickname']) && ( strlen(trim( $_POST['nickname'] ) ) == 0 ) ) {
						$ultimatemember->form->add_error('nickname', __('You must provide your nickname','ultimatemember') );
					}
				}
	...
	function um_account_display_tabs_hook( $args ) {
		...
		?>
			...
			<ul>
				...
				<li<?php echo $title==''?' style="display:none;"':''?>>
