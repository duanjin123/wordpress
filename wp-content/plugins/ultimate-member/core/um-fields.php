...
class UM_Fields {
	...
	function field_label( $label, $key, $data ) {
		...
		if ( $this->viewing == true ) {
			...
			$label = apply_filters("um_edit_label_all_fields", __( $label, UM_TEXTDOMAIN ), $data );
		}
			...
			case 'password':
					...
					if ( $this->set_mode != 'login' && isset( $data['force_confirm_pass'] ) && $data['force_confirm_pass'] == 1 ) {
						...
						$output .= '<div class="um-field' . $classes . '"' . $conditional . ' data-key="'.$key.'">';
								...
								if ( isset( $data['label'] ) ) {
								$output .= $this->field_label( sprintf(__('Confirm %s','ultimatemember'), __($data['label'],'ultimatemember') ), $key, $data);
								}
