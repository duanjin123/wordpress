...
function QAPress_render(){
    ...
    if(isset($wp_query->query['qa_id']) && $wp_query->query['qa_id']){
        return QAPress_single($wp_query->query['qa_id']);
    }else{
...
function QAPress_list( $page = 1 ){
    ...
    $html = '<div class="q-content q-panel"><div class="q-header"><a href="'.QAPress_category_url('').'" class="topic-tab'.($cat==''?' current-tab':'').'">全部</a>';
    if($qa_cats && $qa_cats[0]){
        foreach ($qa_cats as $cid) {
            $c = get_term(trim($cid), 'qa_cat');
            if($c){
                $html .= '<a href="'.QAPress_category_url($c->slug).'" class="topic-tab'.($cat==$c->slug?' current-tab':'').'">'.$c->name.'</a>';
            }
        }
    }
    ...
    if($list){
        foreach ($list as $item) {
            $html .= '<div class="q-topic-item">
                        <a class="user-avatar pull-left" href="javascript:;" data-user="'.$item->user.'">
                            '.get_avatar( $item->user, '60' ).'
                        </a>
                        <div class="reply-count pull-left">
                            <span class="count-of-replies" title="回复数">'.$item->answers.'</span>
                            <span class="count-seperator">/</span>
                            <span class="count-of-visits" title="点击数">'.$item->views.'</span>
                        </div>
                        <div class="last-time pull-right">';
            if($item->last_answer) $html .= '<a class="last-time-user" href="'.QAPress_single_url($item->ID).'#answer" target="_blank">
                                '.get_avatar( $item->last_answer, '60' ).'
                            </a>';
            $html .= '<span class="last-active-time">'.QAPress_format_date(strtotime($item->modified)).'</span>
                        </div>
                        <div class="topic-title-wrapper">'. ($item->flag==1 ? '<span class="put-top">置顶</span>' : '<span class="topiclist-tab">'.QAPress_category($item->category, $all_cats).'</span>')
                            .' <a class="topic-title" href="'.QAPress_single_url($item->ID).'" title="'.esc_attr($item->title).'" target="_blank">'.$item->title.'</a>
                            </div>
                        </div>';
        }
    }else{
...
function QAPress_single( $id ){
    if(!$id) return;
    global $qa_single, $wpcomqadb, $wp_query, $qa_options;
    if(!isset($qa_options)) $qa_options = get_option('qa_options');

    $answers_per_page = isset($qa_options['answers_per_page']) && $qa_options['answers_per_page'] ? $qa_options['answers_per_page'] : 20;

    $question = $qa_single && $qa_single->ID==$id ? $qa_single : $wpcomqadb->get_question( $id );

    if( ! ( $question && isset($question->ID) ) ){
        exit();
    }

    $user = get_user_by('ID', $question->user);
    ...
    $answers = $wpcomqadb->get_answers($id, $answers_per_page, 1, $answers_order);
    $cat = get_term($question->category, 'qa_cat');

    $html = '<div class="q-content q-single" data-id="'.$id.'">
            <div class="q-header topic-header">
                '.($question->flag==1?'<span class="put-top">置顶</span>':'').'
                <h1 class="q-title">'.$question->title.'</h1>
                <div class="q-info">';
    if( current_user_can( 'manage_options' ) ){
        $edit_url = QAPress_edit_url($id);
        $html .= '<div class="pull-right qa-manage">
            <a href="'.$edit_url.'">编辑</a>
            <a class="j-set-top" href="javascript:;">'.($question->flag==1?'取消置顶':'置顶').'</a>
            <a class="j-del" href="javascript:;">删除</a>
        </div>';
    }
    $html .= '<span class="q-author">'.$author_name.'</span>
                    <span class="q-author">发布于 '.QAPress_format_date(strtotime($question->date)).'</span>
            ...
            <div class="q-answer" id="answer">
                <h3 class="as-title">'.$question->answers.'个回复</h3> <ul class="as-list">';
    if($answers){
        foreach ($answers as $answer) {
            ...
            $html .= '<li id="as-'.$answer->ID.'" class="as-item" data-aid="'.$answer->ID.'">
                        <div class="as-avatar">
                            '.get_avatar( $answer->user, '60' ).'
                        </div>
                        <div class="as-main">
                            <div class="as-user">'.$author_name.'</div>
                            <div class="as-content entry-content">'.wpautop($answer->content).'</div>
                            <div class="as-action">
                                <span class="as-time">'.QAPress_format_date(strtotime($answer->date)).'</span>
                                <span class="as-reply-count"><a class="j-reply-list" href="javascript:;">'.$answer->comments.'条评论</a></span>
    ...
    if($question->answers>$answers_per_page){
...
add_action('template_redirect', 'wpcom_pre_process_shortcode');
function wpcom_pre_process_shortcode() {
...
function QAPress_new_question(){
    ...
    if($type=='edit'){
        ...
        if($question && ( $question->user==$current_user->ID || $current_user->has_cap( 'edit_others_posts' ) ) ) { // 问题存在，并比对用户权限
